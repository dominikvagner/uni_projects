 ...
 * @param postfixExpressionLength Ukazatel na aktuální délku výsledného post
 * ixového výrazu
 */
...
void doOperation(Stack *stack, char c, char *postfixExpression,
                 unsigned *postfixExpressionLength) {
  /* Priorita zpracovavaneho operatoru. */
  int c_priority = 0;
  switch (c) {
  case '+':
  case '-':
  ...
  /* Prochazime cyklus do splneni nejake podminky pro vlozeni operatoru. */
  while (1) {
    ...
    /* Kontrola vrcholu zasobniku na levou zavorku. */
    char top;
    Stack_Top(stack, &top);
    if (top == '(') {
      Stack_Push(stack, c);
    ...
    /* Urceni priority operatoru na vrcholu zasobniku. */
    int top_priority = 0;
    switch (top) {
    case '+':
    case '-':
    ...
    /* Porovnani priorit operatoru. */
    if (top_priority < c_priority) {
      Stack_Push(stack, c);
      break;
    } else if (top_priority >= c_priority) {
      Stack_Pop(stack);
      postfixExpression[*postfixExpressionLength] = top;
      (*postfixExpressionLength)++;
    }
 ...
 * ověřte, že se alokace skutečně zdrařila. V případě chyby alokace vraťte namísto ř
 * tězce konstantu NULL.
 *
